...
import { FaqSection as FaqSectionType } from '@/types/sanity/schema';
...
const FAQ = ({
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentIndex((prev) => (prev + 1) % (faqs.length || 1));
    }, 6000);
  ...
  return (
    <section>
      ...
      >
        ...
        {/* Heading */}
        <h1 className="gradient-text text-5xl font-bold mb-4">{heading}</h1>
        ...
        <div className="min-h-60">
          <AnimatePresence mode="wait">
            {faqs.length > 0 && (
              <FaqContent
                key={faqs[currentIndex]?._key || currentIndex}
